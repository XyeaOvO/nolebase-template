...
3.  **第三步：构建归一化切比雪夫低通原型 $H(p)$** 
    ...
    *   **关键：寻找归一化极点 $p_k$**
        $H(p)H(-p)$ 的极点是方程 $1+\epsilon^2 C_N^2(-jp) = 0$ 的根。求解这个方程需要用到 $C_N(x) = \cos(N \arccos x)$ 并进行解析延拓。经过推导，可以得到 N 阶归一化切比雪夫低通滤波器的极点 $p_k = \sigma_k + j\omega_k$：
    ...
    *   **极点分布特点**:
        ...
        *   所有极点都严格位于**左半平面 (LHP)**，因此天然满足稳定性要求。
        *   ![](Pic/Pasted%20image%2020250513121413.png) (图示极点在椭圆上的分布)
    ...
    *   **归一化系统函数 $H(p)$**:
        ...
        增益常数 $K$ 需要根据直流增益来确定：
        *   如果 N 是奇数，$H(0)=1$，则 $K = |\prod_{k=0}^{N-1} (-p_k)|$。
        *   如果 N 是偶数，$H(0)=1/\sqrt{1+\epsilon^2}$，则 $K = \frac{|\prod_{k=0}^{N-1} (-p_k)|}{\sqrt{1+\epsilon^2}}$。
        在实际计算中，更常用的是考虑 $H(p)H(-p)$ 在 $p=0$ 的值。$H(p)H(-p)|_{p=0} = 1/(1+\epsilon^2 C_N(0)^2)$。而 $H(p) = K / \prod(p-p_k)$。$H(p)H(-p) = K^2 / (\prod(p-p_k)\prod(-p-p_k))$. 令 $p=0$, 得 $K^2 / (\prod(-p_k)\prod(-p_k)) = K^2 / (\prod p_k^2) = 1/(1+\epsilon^2 C_N(0)^2)$. 也可以用最高次项系数来定标。分母最高次项是 $p^N$。$C_N(-jp)$ 的最高次项是 $(2^{N-1})(-jp)^N$。$H(p)H(-p) \approx 1 / (\epsilon^2 (2^{N-1})^2 (-j)^{2N} p^{2N})$。$H(p) \approx K/p^N$。$H(-p) \approx K/(-p)^N$。$H(p)H(-p) \approx K^2 / ((-1)^N p^{2N})$。比较系数可以得到 $K$。一个简化的结果是，如果将 $H(p)$ 的分母多项式写作 $D(p) = \prod (p-p_k) = p^N + d_{N-1}p^{N-1} + \dots + d_0$，则分子常数 $K = d_0$ (如果N奇) 或 $K = d_0/\sqrt{1+\epsilon^2}$ (如果N偶)。更标准的做法是利用最高项系数，分子常数应为 $\prod (-p_k)$ 乘以一个可能的 $1/\sqrt{1+\epsilon^2}$ 因子。或者直接看 $H(p)H(-p)$ 的分子是1，其分母最高次项系数是 $\epsilon^2 (2^{N-1})^2 (-1)^N$，所以 $H(p)$ 的分母最高次项 $p^N$ 的系数为 1，分子常数应使得在 $p \to \infty$ 时， $H(p)H(-p) \approx K^2/ (p^{2N}) \approx 1 / (\epsilon^2 (2^{N-1})^2 (-1)^N p^{2N})$. 这表明 $K$ 可能与 $\epsilon$ 和 $N$ 有关。在实际查表或使用软件时，增益常数通常已包含在内。
...
2.  **查阅归一化衰减特性图确定阶数 N**:
    ...
    *   ![](Pic/Pasted%20image%2020250501151058.png) (示例图表)
...
3.  **查表确定归一化低通原型 $H(p)$ 的极点或分母多项式/因式**:
    ...
    *   ![](Pic/Pasted%20image%2020250501151208.png) (示例表格片段)
    *   **注意表格中的归一化增益**: 表格通常会给出标准形式，比如 $H(p) = K / D(p)$，并指明 $K$ 的取值（或者直接给出包含 $K$ 的分子多项式，对于全极点滤波器 K 是常数）。要确保直流增益或通带波纹特性符合预期。
...
Matlab 提供了专门的函数来设计切比雪夫 I 型模拟滤波器：

*   `[n, Wn] = cheb1ord(Wp, Ws, Rp, Rs, 's')`:
    *   功能: 计算满足指标的切比雪夫 I 型滤波器的最低阶数 `n` ($N$) 和通带截止频率 `Wn` ($\Omega_p$)。
    *   参数: `Wp`, `Ws`, `Rp`, `Rs` 含义同 `buttord`，`'s'` 指明模拟。
*   `[z, p, k] = cheb1ap(n, Rp)`:
    *   功能: 计算 `n` 阶、通带波纹为 `Rp` dB 的 **归一化** (即 $\Omega_p=1$ rad/s) 切比雪夫 I 型模拟低通滤波器的零点 `z` (空)、极点 `p` 和增益 `k`。
*   `[b, a] = cheby1(n, Rp, Wn, 's')`:
    *   功能: **一步到位**设计出 $N$ 阶、通带波纹 `Rp` dB、通带截止频率为 `Wn` ($\Omega_p$) 的切比雪夫 I 型模拟低通滤波器。
    *   `'ftype'` (可选): 同样支持 `'low'`, `'high'`, `'bandpass'`, `'stop'`。

**使用流程示例 (推荐)**:
1.  用 `cheb1ord` 确定阶数 `n` 和通带截止频率 `Wn`。
    `[n, Wn] = cheb1ord(Omega_p, Omega_s, Ap, As, 's');`
2.  用 `cheby1` 直接得到滤波器系数。
...
*   **设计总结图**: ![](Pic/Pasted%20image%2020250501151804.png) (汇总了指标、公式、极点分布和最终函数的关系)
...
*   **Matlab 实现**:
    *   `[n, Wn] = ellipord(Wp, Ws, Rp, Rs, 's')`: 计算椭圆滤波器的最低阶数 `n` 和通带截止频率 `Wn` ($\Omega_p$)。
    *   `[z, p, k] = ellipap(n, Rp, Rs)`: 计算归一化椭圆低通原型 ($\Omega_p=1$) 的零点 `z`、极点 `p` 和增益 `k`。注意椭圆滤波器**不是**全极点滤波器，它在虚轴上有零点，这些零点导致了阻带的波纹。
    *   `[b, a] = ellip(n, Rp, Rs, Wn, 's')`: 一步到位设计椭圆低通滤波器。
    *   `'ftype'` (可选): 支持 `'low'`, `'high'`, `'bandpass'`, `'stop'`。
...
*   **图形比较**:
    ![](Pic/Pasted%20image%2020250501152751.png) (直观展示三种滤波器的幅度响应差异)
...
*   **图形示意**:
    *   LP $\to$ LP': 只是频率轴的线性伸缩。![](Pic/Pasted%20image%2020250501153530.png)
    *   LP $\to$ HP/BP/BS: 涉及更复杂的非线性频率轴映射。
        ![](Pic/Pasted%20image%2020250501153552.png)
...
*   **非对称 BPF 设计步骤 (调整阻带)**:
    ...
    7.  进行后续设计。
    *   ![](Pic/Pasted%20image%2020250501153916.png) (图示调整 BPF 阻带)
...
*   **非对称 BSF 设计步骤 (调整通带)**:
    ...
    7.  进行后续设计。
    *   ![](Pic/Pasted%20image%2020250501154021.png) (图示调整 BSF 通带)
...
*   **例1: 设计巴特沃思带阻滤波器 (BSF)** (接自 PDF Page 31-32)
    1.  **目标指标**: $f_{s1}=3.8M, f_{s2}=4.8M, A_s=20dB$; $f_{p1}=3.1M, f_{p2}=5.5M, A_p=3dB$。
    2.  **对称性检查与调整**: 已在 PDF Page 31 计算，不满足对称。调整通带边缘得到 $\bar{\Omega}_{p1}=2\pi(3.3M), \Omega_{p2}=2\pi(5.5M)$。最终指标 $A_p=3dB, A_s=20dB$。
    3.  **计算 LPF 原型指标**:
        *   $A_p=3dB, A_s=20dB$。
        *   $\lambda_p = 1$。
        *   $\lambda_s = (\Omega_{p2}-\bar{\Omega}_{p1}) / (\Omega_{s2}-\Omega_{s1}) = (5.5-3.3)/(4.8-3.8) = 2.2/1.0 = 2.2$。
    4.  **设计归一化 LPF (Butterworth)**:
        *   已在 PDF Page 32 计算，需要 $N=3$。
        *   $A_p=3dB \Rightarrow \Omega_c = \Omega_p$ (对于原型, $\lambda_c=1$)。
        *   $H_{LP}(p) = \frac{1}{p^3+2p^2+2p+1}$。
    5.  **执行频率变换 (LP $\to$ BS)**:
        *   $B = \bar{\Omega}_{p2}-\bar{\Omega}_{p1} = 2\pi(2.2M)$。
        *   $\Omega_0^2 = \Omega_{s1}\Omega_{s2} = (2\pi)^2(3.8M)(4.8M)$。
        *   变换公式: $p = \frac{B s}{s^2 + \Omega_0^2}$。
    6.  **代入与化简**: 将 $p$ 的表达式代入 $H_{LP}(p)$，得到 $H_{BS}(s)$ (最终表达式见 PDF Page 32)。

*   **例2: 设计带通滤波器 (BPF)** (接自 PDF Page 33-35)
    1.  **目标指标**: $A_p=1dB, A_s=40dB, f_{p1}=1510, f_{p2}=1570, f_{s1}=1394, f_{s2}=1704$。
    2.  **对称性检查与调整**: 已在 PDF Page 33 计算，近似对称但做了调整。$\bar{\Omega}_{s2}=2\pi(1700.6)$。最终指标 $A_p=1dB, A_s=40dB$。
    3.  **计算 LPF 原型指标**:
        *   $A_p=1dB, A_s=40dB$。
        *   $\lambda_p = 1$。
        *   $\lambda_s = (\bar{\Omega}_{s2}-\Omega_{s1})/(\Omega_{p2}-\Omega_{p1}) = (1700.6-1394)/(1570-1510) = 306.6/60 \approx 5.11$。
    4.  **设计归一化 LPF**:
        *   **Butterworth**: 已在 PDF Page 34 计算，需要 $N=4$。$H_{LP}(p) = \frac{1}{(p^2+0.7654p+1)(p^2+1.8478p+1)}$。
        *   **Chebyshev**: 已在 PDF Page 35 计算，需要 $N=3$。$H_{LP}(p) = \frac{0.4913}{(p^2+0.4942p+0.9942)(p+0.4942)}$。
    5.  **执行频率变换 (LP $\to$ BP)**:
        *   $B = \Omega_{p2}-\Omega_{p1} = 2\pi(60)$。
        *   $\Omega_0^2 = \Omega_{p1}\Omega_{p2} = (2\pi)^2(1510)(1570)$。
        *   变换公式: $p = \frac{s^2 + \Omega_0^2}{B s}$。
    6.  **代入与化简**: 将 $p$ 的表达式分别代入 Butterworth 和 Chebyshev 的 $H_{LP}(p)$，得到各自的 $H_{BP}(s)$。

#### 9.4 借助现代工具：Matlab 实现频率变换 💻

Matlab 提供了直接进行频率变换的函数，无需手动代入：

*   `[bt, at] = lp2lp(b, a, Wo)`: 低通到低通 (b, a 是 LPF 原型系数, Wo 是目标 LPF 截止频率 $\Omega_p'$)。
